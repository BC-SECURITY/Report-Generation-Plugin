...
class Attack(object):
    ...
    def parse_json(self, mitre_json):
        ...
        for i in range(len(mitre_json['techniques'])):
            ...
            technique_list.append(mitre_json['techniques'][i]['techniqueID'].split('.')[0])
    ...
    def threat_filtering(self, threat_name):
        ...
        for i in range(len(techniques)):
            # Finds techniques from group and filters out the sub-techniques
            # TODO:  cannot do subtechniques so split
            technique_list.append(techniques[i]._inner['external_references'][0]._inner['external_id'].split('.')[0])
        disable_module_count = self.disable_modules(technique_list)
        return disable_module_count
    ...
    def load_database(self):
        try:
            ...
            if not os.path.isfile(self.mainMenu.installPath + '/data/cti/enterprise-attack'):
                urllib.request.urlretrieve('https://github.com/mitre/cti/archive/refs/tags/ATT&CK-v8.2.tar.gz', filename=database_tar)
                if database_tar.endswith("tar.gz"):
                    tar = tarfile.open(database_tar, "r:gz")
                    tar.extractall(path=self.mainMenu.installPath + '/data')
                    tar.close()
            fs = FileSystemSource(self.mainMenu.installPath + '/data/cti-ATT-CK-v8.2/enterprise-attack')
            return fs
